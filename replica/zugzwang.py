...
from model import Variation

engine_limit = chess.engine.Limit(depth = 30, time = 10, nodes = 12_000_000)

def zugzwang(engine: SimpleEngine, variation: Variation) -> bool:
    for node in variation.mainline[1::2]:
        ...
        score = score_of(engine, board, not variation.pov)

        rev_board = node.board()
        rev_board.push(Move.null())
        rev_score = score_of(engine, rev_board, not variation.pov)
